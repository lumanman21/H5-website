...
function filter_pre_get_posts( $query ){
    ...
    if ( is_category() ) {
        ...
        if ( $cat->term_id === 2 || cat_is_ancestor_of( 2, $cat->term_id ) ) {
            $num = 12;
        }
        if ( $cat->term_id === 3 || cat_is_ancestor_of( 3, $cat->term_id ) ) {
            $num = 6;
        }
    }
...
// 1. 固定链接更新函数
function my_update_permalinks(){
    flush_rewrite_rules();
    wp_cache_flush();
}
// 2. 检查是否需要更新
function my_maybe_update_permalinks(){
    // 获取上次更新时间
    $last_updated = get_option('my_permalinks_last_updated');
    // 如果是第一天访问或者距离上次更新超过1天
    if(!$last_updated || $last_updated < strtotime('-1 days')){
        // 执行更新
        my_update_permalinks();
        // 更新保存时间
        update_option('my_permalinks_last_updated', time());
    }
}
// 3. init动作触发检查更新
add_action('init', 'my_maybe_update_permalinks');